...
CREATE TABLE IF NOT EXISTS MyData(accessToken text not null, nickName text not null, age int not null, gender text not null, bloodType text not null, imageUrl text not null, chatLevel int not null, gentle int not null, cool int not null, pervert int not null, common int not null, nickNameTag text )

INSERT INTO UserProfiles(id,nickName, age, gender, bloodType, imageUrl, chatLevel, gentle, cool, pervert,common ) VALUES()
