...
CKNetworkManager *networkmanager_instance;
...
+(CKNetworkManager *)getInstance {
    if(!networkmanager_instance){
        networkmanager_instance = [[CKNetworkManager alloc] init];
    }
...
-(BOOL)socketInitializeWithUserData:(CKUserData *)userData withHost:(NSString *)serverHost withPort:(UInt32)port{
...
void socketOpened(){
    [[CKNetworkManager getInstance]setStatus:CONNECTED];
    sendAuth();
}
void socketClosed(){
    [[CKNetworkManager getInstance]setStatus:CLOSED];
}
void socketError(){
    socket_finalize();
    socketClosed();
...
void messageComplete(int header, CFDictionaryRef cfDictionary){
    ...
    switch (header) {
        ...
        case 0x1002: // session okay
            [[CKNetworkManager getInstance]setStatus:AUTHORIZED];
            break;
        ...
        case 0x2012: // seding message successful.
            ...
            [[CKMessageDispatcher getInstance] sendSuccess:(long) datetime];
            break;
        case 0x2013: // seding message failed.
            //todo Error Handling
            break;
        case 0x2101: // new message arrival
            [[CKMessageDispatcher getInstance] newChatMessage:dictionary];
            break;
        ...
        case 0x4101: // enqueue successful.
            [[CKRandomChatManager getInstance] enqueueSuccessfully];
            break;
        case 0x4111: // enqueue failed : already in queue
        case 0x4112: // enqueue failed : match count limit excessed.
            [[CKRandomChatManager getInstance] failed:dictionary];
            break;
        case 0x4201: // match established
            [[CKRandomChatManager getInstance] matchEstablished:dictionary];
            break;
...
void sendMsg(NSString* address, NSString* msg){
    [[CKSampleDataGenerator getInstance] messageWillSend:msg toAddress:address];
}
