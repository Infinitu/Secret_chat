...
@interface CKFriendManager ()
@property NSOperationQueue *queue;
@end

@implementation CKFriendManager
CKFriendManager *FriendManager_instance;
...
+ (CKFriendManager *)getInstance {
    if(!FriendManager_instance)
        FriendManager_instance = [[CKFriendManager alloc] init];
...
- (void)sendFriendResponseTo:(NSString *)address withAction:(NSString *)action {
    ...
    sendMessage(0x5002,
            (uint8_t *) [[NSString stringWithFormat:@"%@|%@|",address,action]
                    cStringUsingEncoding:NSUTF8StringEncoding]);
    CKFriendRequest *req = [CKFriendRequest objectForPrimaryKey:address];
...
-(void)requestReceivedWithData:(NSDictionary *)data {
    ...
    [self getUserInfomation:address WithCallback:^(NSDictionary *dictionary) {
        CKFriendRequest *request = [[CKFriendRequest alloc] init];
        ...
        [[RLMRealm defaultRealm] beginWriteTransaction];
        [CKFriendRequest createInDefaultRealmWithValue:request];
        [[RLMRealm defaultRealm] commitWriteTransaction];
        ...
        if([appDelegate isInBackground]){
            [[CKNotificationManager getInstance] pushNotification:[NSString stringWithFormat:@"%@님이 새로운 친구요청을 보내셨습니다.",request.nickname]
...
-(void)establishedReceivedWithData:(NSDictionary *)data {
    ...
    [self getUserInfomation:address WithCallback:^(NSDictionary *dictionary) {
        CKFriend *newFriend = [[CKFriend alloc]init];
        ...
        [[RLMRealm defaultRealm] beginWriteTransaction];
        [CKFriend createInDefaultRealmWithValue:newFriend];
        [[RLMRealm defaultRealm] commitWriteTransaction];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"FriendListChanged" object:self];
        AppDelegate *appDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
        if([appDelegate isInBackground]){
            [[CKNotificationManager getInstance] pushNotification:[NSString stringWithFormat:@"%@님이 새로운 친구로 등록되었습니다.",newFriend.nickname]
