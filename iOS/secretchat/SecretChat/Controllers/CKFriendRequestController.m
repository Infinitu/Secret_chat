...
#import "Version.h"
#import "CKMasterTabBarController.h"
#import "CKFriendRequestTableViewCell.h"
#import <SDWebImage/UIImageView+WebCache.h>
#import "CKUserData.h"
#import "CKJsonParser.h"
#import "CKFriendManager.h"

@interface CKFriendRequestController ()
...
@end
@implementation CKFriendRequestController
...
-(void)refreshObjects{
    ...
    RLMResults *result = [CKFriendRequest allObjects];
    NSInteger cnt = [result count];
    for(CKFriendRequest *req in result)
...
-(void)AcceptFriends:(CKFriendRequest *)req{
    [[CKFriendManager getInstance] sendFriendResponseTo:req.address withAction:@"accept"];
//    [[RLMRealm defaultRealm] beginWriteTransaction];
//    [CKFriendRequest delete:req];
...
-(void)DenyFriends:(CKFriendRequest *)req{
    [[CKFriendManager getInstance] sendFriendResponseTo:req.address withAction:@"deny"];
//    [[RLMRealm defaultRealm] beginWriteTransaction];
//    [CKFriendRequest delete:req];
//    [[RLMRealm defaultRealm] commitWriteTransaction];
//    [self refreshObjects];
...
- (IBAction)addFriends:(id)sender {
    [[CKFriendManager getInstance] sendFriendRequestTo:self.userFound[@"_id"] withMessage:nil]; //todo!!
    [self hideResultWindow];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CKFriendRequest *obj = self.arrivedRequest[(NSUInteger) indexPath.row];
    CKFriendRequestTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"friendRequest"];
    cell.caller=self;
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        [[RLMRealm defaultRealm] beginWriteTransaction];
        [CKFriendRequest delete:self.arrivedRequest[(NSUInteger) indexPath.row]];
        [[RLMRealm defaultRealm] commitWriteTransaction];
